...
void World::initWorld() 
{
  ...
  soundManager->playSound(soundManager->ambience, soundManager->ambienceSource, true);
  soundManager->setVolume(soundManager->ambienceSource, 0.5f);
...
void World::checkCollisions()
{
  ...
  if (this->state != STATE::GAME_OVER)
  {
    for (GLint i = 0; i < this->pawns.size(); i++) 
    {
      if (this->pawns.at(i)->assignedTo == this->agent->currentRow) 
      {
        ...
        if (this->pawns.at(i)->honked == 0)
        {
          ...
          if (hornResult) {
            ...
            if (rand() % 2 == 0) {
              ...
              switch (hornType) {
              case 0:
                soundManager->playSound(soundManager->horn1, soundManager->effectSource);
                break;
              case 1:
                soundManager->playSound(soundManager->horn2, soundManager->effectSource);
                break;
              case 2:
                soundManager->playSound(soundManager->horn3, soundManager->effectSource);
                break;
              case 3:
                soundManager->playSound(soundManager->horn4, soundManager->effectSource);
                break;
              default:
                break;
              }
        ...
        {
          soundManager->setVolume(soundManager->ambienceSource, 0.15f);
          soundManager->playSound(soundManager->gameOver, soundManager->effectSource);
          soundManager->playSound(soundManager->carHit, soundManager->effectSource);
  ...
  for (GLint i = 0; i < this->coins.size(); i++)
  {
    ...
    if (result)
    {
      soundManager->playSound(soundManager->coinPick, soundManager->effectSource);
